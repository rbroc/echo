...
def create_corrM(df, save_dir=None, file_name=None):
...
def main(): 
    ...
    create_corrM(train_df[features], savedir, f"heatmap_temp{temp}.png")

    # subset features 
    subset_features = ["alpha_ratio", "proportion_unique_tokens", "n_sentences", "n_unique_tokens", "doc_length", "rix", "gunning_fog", "pos_prop_NOUN", "pos_prop_NUM", "perplexity", "sentence_length_mean", "proportion_ellipsis", "pos_prop_SCONJ", "syllables_per_token_mean", "dependency_distance_mean", "pos_prop_VERB", "flesch_reading_ease", "mean_word_length", "oov_ratio", "entropy"]
    subset_features = sorted(subset_features)

    create_corrM(train_df[subset_features], savedir, f"heatmap_temp{temp}_subset.png")
