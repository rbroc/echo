...
def main():
    ...
    datapath = path.parents[2] / "raw_datasets" / "text" / "human_datasets" / dataset
...
def hf_pipeline(args, df, min_len, max_tokens, path, chosen_model_name, cache_models_path=None, sample_params:dict=None):    
    ...
    if args.data_subset is None:
        outpath = path.parents[2] / "raw_datasets" / "text" / "ai_datasets" / "HF" / "FULL_DATA" / chosen_model_name
    else:
        outpath = path.parents[2] / "raw_datasets" / "text" / "ai_datasets" / "HF" / "SUBSET_DATA" / chosen_model_name
...
def vllm_pipeline(args, df, min_tokens, max_tokens, path, chosen_model_name, cache_models_path=None, sample_params:dict=None):
    ...
    '''
    if args.data_subset is None:
        outpath = path.parents[2] / "raw_datasets" / "text" / "ai_datasets" / "vLLM" / "FULL_DATA" / chosen_model_name
    else:
        outpath = path.parents[2] / "raw_datasets" / "text" / "ai_datasets" / "vLLM" / "SUBSET_DATA" / chosen_model_name
