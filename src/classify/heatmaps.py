...
def main(): 
    ...
    for dataset in ["stories", "dailymail_cnn", "mrpc", "dailydialog"]:
        for temp in [1, 1.5]:
            ...
            # filter 
            df = filter_metrics(df, percent_NA=0.9, percent_zero=0.9, verbose=False, log_file=path.parents[0] / "heatmaps" / "filtered_metrics_log.txt")

            create_corrM(df, f"{dataset.upper()} (Temperature of {temp})", path.parents[0] / "heatmaps", f"{dataset}_heatmap_temp{temp}.png")
